...
| `producto` | Picks a **product**; gets an **immediate quote** | **Buttons** if ≤ 3 products, else **list** (≤ 10). Catalog: the 9 fixed SKUs (`src/src/catalog/skus.ts`) → **use a list** (>3 items, 9 ≤ 10 ✅). Dispensers are NOT products (they come with the abono/comodato). | If product already free-typed, skip the picker; AI quotes directly. Prices come ONLY from `get_prices(city)`, that city's list only (`00-master.md` §6). |
...
- **Catalog**: the 9 fixed SKUs, identical in every zone, derived from `src/src/catalog/skus.ts` — Botellón 20L, Botellón 12L, Botellón 20L Bajo Sodio, Botellón 12L Bajo Sodio, Soda en Sifón 1,5L, Agua Saborizada 1,5L, Gaseosa 2L, Agua 2L, Isotónica CIMES 750mL. A price list supplies PRICES ONLY; anything else in it (dispensers, abonos, sanitización) is filtered out.
