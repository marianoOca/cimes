...
| `POST /api/leads` | Body: `{ source: "web", name, phone, city, address, cross_streets?, ...attribution }`. Early capture from the website wizard's first step, before a cart or a coverage result exists. Creates/updates the lead by phone and sets `stage: "datos_entrega"` — **nothing else**: no sheet row, no CRM mirror, no WaterService call, no follow-up timers. Response: `{ ok: true }`. Idempotent per phone. |
