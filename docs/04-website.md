...
7. **Coverage areas** — feature the main service cities: **Mercedes, Luján, San Andrés de Giles, San Antonio de Areco, Chivilcoy, Campana, Zárate**. Static display (marketing reassurance; **any Buenos Aires province city is served** — the authoritative coverage check is the live per-address one in the wizard, §3 step 4).
...
### City select — before the wizard, not a step in it

The visitor picks a city on the home page or on a bare `/alta/`, and that choice **navigates** to `/alta/?city=<slug>` where the wizard proper begins. It carries no step number and no stepper: nothing is collected, and there is nothing to go back to.

They pick one of the 8 quick-pick cities, or use **"Otra ciudad"** — an inline **free-text field with a native autocomplete** (`GET /api/cities`, ~130 BA cities); its **Continuar** button stays disabled until a city is typed. On submit the typed city is resolved via **`POST /api/resolve-city`**, with two outcomes:

- **Recognized** (exact or a typo within the match floor, e.g. `nechochea`) → snapped to the canonical city and the wizard **continues the normal flow** (data → dispenser → catalog → coverage/day → summary) exactly like a quick-pick.
...
1. **Delivery-data form.** The first screen a `?city` link lands on. Collect: **nombre**, **apellido**, **teléfono (WhatsApp)**, **dirección**, **piso/depto** (optional), **entre calles** (optional). Apply **client-side validation** (required fields, phone format) before proceeding. (Server-side validation is enforced independently by the endpoints — see "Validation" below.)

   The city is already known (it's in the URL), so it is shown as a **header row above the form** — `Ciudad: <city>` on the left, a **Cambiar** link on the right (brand red) back to the picker — rather than occupying a step of its own. Cambiar is a real link to `/alta/` (carrying any UTMs), not an in-place re-render: `?city=` is still in the URL, so re-rendering the picker in place would be undone by the next reload.

   **On submit the lead is saved**, via **`POST /api/leads`** (§4) — name, phone, city, composed address, cross streets, plus attribution. This is why the step moved to the front: before it did, nothing about the visitor reached the backend until the coverage call four screens later, so everyone who dropped out in between was lost. The call is **fire-and-forget** — the wizard neither waits on it nor surfaces a failure, and nothing downstream depends on it having landed. It is keyed by phone server-side, so going Back and re-submitting updates one lead rather than making a second.
...
2. **Dispenser + water type.** Two decisions on one screen, because to the visitor they are one decision — and together they pick the price list, the catalog, and the abono. Three cards (two side by side, one full-width, mirroring the competitor layout): **Dispenser Natural** (free, comodato), **Dispenser Frío/Calor** (comodato + monthly abono, 50% off the first month), **Sin dispenser**. The two dispenser cards carry an inline **Común / Bajo en sodio** toggle, defaulting to *Común*; *Sin dispenser* has none.
   ...
   This step also performs the **`GET /api/prices?city=<city>`** call — the frío/calor card needs its prices, and the city is already known. Every number on the card comes from the response's `frio_calor` block; **the site hardcodes no price**. When that block is `null` — the deployment can't price the abono for this city — the **Frío/Calor card is not offered at all** rather than quoted with a guess.
...
A same-tab reload **resumes the furthest step reached** for that city. **Changing city keeps the delivery data and drops everything after it** — who someone is and where they live doesn't change with the city, and making them retype a name and phone is the one thing the Cambiar button must not cost; the dispenser choice, cart and delivery option are all priced or routed per city, so those go.
...
|---|---|
| `GET /api/cities` | City picker (pre-wizard). Returns the ~130 BA-province cities for the "Otra ciudad" free-text autocomplete. |
| `POST /api/resolve-city` | City picker (pre-wizard). Snaps the typed "Otra ciudad" text to the closest real BA city; the wizard then continues the normal flow. |
| `POST /api/leads` | Wizard step 1 (on submit). Body: `name`, `phone`, `city`, `address` (composed), optional `cross_streets`, `source: "web"`, plus attribution. **No `items` and no dispenser fields** — neither exists yet. Saves the half-finished signup so an abandoned wizard stays reachable; keyed by phone, so a repeat submit updates one lead. Fire-and-forget: the site ignores the response and never blocks on it. |
...
| `POST /api/orders` | Wizard step 5 (confirm). Body: full order — `name` (nombre + apellido), `phone`, `city`, `address` (calle + altura), `cross_streets` (entre calles), `product`, optional **`dispenser`** (`natural\|frio_calor\|ninguno`) + **`water_type`** (`comun\|bajo_sodio`) from step 2, chosen `delivery_day` + `delivery_window`, and **`source: "web"`**. With `dispenser: "frio_calor"` an empty `items` is valid — the abono is the order. Triggers the shared confirmation pipeline (see `01-core-api.md`). Idempotent per lead — a double-submit of the same order must not create two altas. Site must guard the confirm button against double-clicks regardless. |
...
The Dirección field (step 1) uses Google Places for suggestions but **must never be a hard dependency** — a plain typed address always works, because the backend geocodes the raw string via WaterService #12 (`01-core-api.md` §3). Three constraints below — each is the fix for a real regression that shipped and broke this field. **Do not undo them.**

1. **Bind from the loader's `callback=` parameter — never from the script's `onload`.** `loading=async` requires a callback, and it is the only hook Maps guarantees to fire *after* the API is initialized. At `onload`, `google` and `google.maps` already exist but **`google.maps.importLibrary` is still `undefined`**, so a bind attempted there silently does nothing and the field stays **unbound forever** — no dropdown, no error, no network call. That is the exact symptom to watch for, and it is invisible in tests (jsdom has no Maps SDK). We load with `&callback=__cimesMapsReady`, which then does `importLibrary("places").then(attachPlaces)`.

   This was masked for a long time: Maps used to be preloaded at the *product* step, so by the time the address step rendered one step later, its **synchronous** `attachPlaces()` call found a fully-materialized SDK and did the real binding. The broken `onload` path never mattered. Moving the address form to step 1 removed that head start and the field stopped autocompleting entirely.

   The address field now renders on the **first** screen, so there is nothing to preload from: `loadGoogleMaps()` and a (no-op) `attachPlaces()` both fire on the data step's render, and the real bind comes from the callback. `attachPlaces()` is idempotent per input (`data-pacBound`), so binding late is safe and binding twice is a no-op. Page-load-eager loading is still wrong — it spends the payload on landing-page bouncers who never open the wizard.

   **Do not `await` the city geocode before attaching the input listener.** The 20 km box (constraint 3) needs a geocode, but awaiting it inline attaches the listener ~1s late and **silently swallows everything typed in the meantime** — which on a first screen is most first-time visitors. Kick the geocode off unawaited and `await` it inside the query function instead, so the first suggestion request is still restricted, just marginally slower.
...
| `GOOGLE_MAPS_KEY` | placeholder `"GOOGLE_MAPS_KEY"` | **website** | Browser Maps key for the step-1 address autocomplete. Needs **Maps JavaScript API + Places API (New)** enabled and **HTTP-referrer-restricted** to the site's domain. **Separate from the backend's server-side `GOOGLE_MAPS_API_KEY`** — never put the server key here (it would be publicly exposed). While the value is the placeholder, the field stays a plain text input (autocomplete no-ops). Local dev injects the backend key via `dev.sh` (`MAPS_KEY`). |
